"""
Standalone training script for Network Congestion Prediction.
Replicates the pipeline from congestion_vf_forecasting_notebook.ipynb
and exports the trained RandomForest model as a .pt file.
"""
...
MODEL_PATH = os.path.join(SCRIPT_DIR, "congestion_rf_model.pt")
...
# Encode scenario as binary feature
...
# Forecasting target (5-step ahead)
FORECAST_HORIZON = 5
fe["future_congestion_label"] = fe.groupby("node_id")["congestion_label"].shift(
    -FORECAST_HORIZON
)

# Fill NaNs from lag/rolling operations
# Note: pandas>=3.0 drops group-key column from groupby().apply()
# so we save node_id before the operation and restore it after.
