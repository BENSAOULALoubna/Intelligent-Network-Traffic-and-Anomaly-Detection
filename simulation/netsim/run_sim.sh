...
NETSIM_DIR="/home/opp_env/default_workspace/netsim"
INET_SRC="/home/opp_env/default_workspace/inet-4.5.4/src"
RESULTS="$NETSIM_DIR/results"
DASHBOARD_DIR="/home/opp_env/netsim_dashboard"
...
case "$MODE" in
    ...
    realtime)
        ...
        if [ "$HOSTS" = "all" ]; then
            ...
            run_config "Realtime_Hosts5"  "$NETSIM_DIR/configs/normal.ini"
            sleep 2   # stagger starts to avoid port conflicts
            run_config "Realtime_Hosts10" "$NETSIM_DIR/configs/normal.ini"
            sleep 2
            run_config "Realtime_Hosts20" "$NETSIM_DIR/configs/normal.ini"
            sleep 2
            run_config "Realtime_Hosts40" "$NETSIM_DIR/configs/normal.ini"
        else
            # Single host count
            run_config "Realtime_Hosts${HOSTS}" "$NETSIM_DIR/configs/normal.ini"
        fi
        echo ""
    ...
    batch)
        ...
        run_config "Batch_Hosts${BATCH_HOSTS}" "$NETSIM_DIR/configs/normal.ini"
